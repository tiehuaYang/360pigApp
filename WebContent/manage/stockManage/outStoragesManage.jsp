			...
			<div id="page-wrapper">
				...
				<div id="title-breadcrumb-option-demo" class="page-title-breadcrumb">
					<div class="page-header pull-left">
						<div class="page-title">库存出库列表</div>
					</div>
											...
											<div class="portlet box">
												<div class="portlet-header">
													<div class="row">
														<div class="col-lg-6">
															<div class="pagination-panel">
																每次浏览&nbsp; <select name="pageSize"
																	...
																	id="billCode" type="text" name="billCode"
																	placeholder="出库单号"   class="form-control input-sm input-inline" style="width:120px;"/>  <input type="button"
														...
														<div class="actions">
															<a href="${contextPath}/manage/newOutStorages"
												...
												<div class="portlet-body">
													<table
														...
														<tbody>
															<c:forEach var="outStoragesVO"
																items="${obj.outStoragesList}" varStatus="status">
																<%-- <form id="${inventoryVO.goodId.replace('-','')}"> --%>
																<tr>
																	<td>${outStoragesVO.billCode}</td>
																	<td><fmt:formatDate
																			value="${outStoragesVO.createTime}" type="both" /></td>
																	<td>${outStoragesVO.storageType}</td>
																	<td>${outStoragesVO.billMaker}</td>
																	
																	<td>
																		<button type="button" class="btn btn-default btn-xs"
																			onclick="showBills('${outStoragesVO.billCode}',0)">
				...
				<script type="text/javascript">
					...
					function edit(tdId, val1, val2) {
						//$("#saveBtn").show();
						document.getElementById(tdId).innerHTML = "<input type='text' id='"
								+ tdId.substr(0, 8)
								+ "' name='maxQuantity' style='width:100%' value='"
								+ val1 + "'>";
						document.getElementById(tdId.toUpperCase()).innerHTML = "<input type='text' id='"
								+ tdId.substr(0, 16)
								+ "' name='minQuantity' style='width:100%' value='"
								+ val2 + "'>";
					}

					function input_submit(id) {
						//alert(id);
						var maxQuantity = document.getElementById(id.substr(0,
								8)).value;
						var minQuantity = document.getElementById(id.substr(0,
								16)).value;
						//alert(maxQuantity);
						$
								.post(
										'${contextPath}/manage/saveStock',
										{
											"accessoryId" : id,
										...
										function(data) {
											...
											if (result.msg == 'OK') {
												alert('保存成功');
												document.getElementById(id).innerHTML = "<input type='hidden' id='"
														+ id.substr(0, 8)
														+ "' name='maxQuantity' style='width:100%'>"
														+ maxQuantity;
												document.getElementById(id
														.toUpperCase()).innerHTML = "<input type='hidden' id='"
														+ id.substr(0, 16)
														+ "' name='minQuantity' style='width:100%'>"
														+ minQuantity;
												//$("#saveBtn").hide();
												return true;
											}
					...
					function delAccessory(id) {
						if (window.confirm("确定要删除这条记录吗？")) {
							var url = "${contextPath}/manage/delAccessory?accessoryId="
									+ id;
					...
					function showAccessory(id) {
						var url = "${contextPath}/manage/editAccessory?accessoryId="
								+ id;
					...
					function doSearch(caseNum) {
						var caseWord = [ "搜商品&nbsp;", "搜订货单&nbsp;",
								"搜退货单&nbsp;", "搜客户&nbsp;", "搜库存&nbsp;" ];
						document.getElementById("searchBtn").innerHTML = caseWord[caseNum]
								+ "<span class='caret'></span>";
					...
					function showSearch(currentPage) {
						...
						if (caseOfSearch == 0) {//搜商品
							url = "${contextPath}/manage/showAccessory?currentPage="
									+ currentPage
									+ "&pageSize="
									+ pageSize
									+ "&keyword=" + keyword;
						} else if (caseOfSearch == 4) {//搜库存
							url = "${contextPath}/manage/showStock?currentPage="
									+ currentPage
									+ "&pageSize="
									+ pageSize
									+ "&keyword=" + keyword;
					...
					function showBills(billCode, storageKind) {
						var url = "${contextPath}/manage/showBills?billCode="
								+ billCode + "&storageKind=" + storageKind;
						window.location.href = url;
					}
					...
					function jumpPage(currentPage) {
						var pageSize = $(
								"select[name=pageSize] option:selected").val();
						var url = "${contextPath}/manage/outStorages?currentPage="
					...
					function showSelect(currentPage) {
						...
						var pageSize = $(
								"select[name=pageSize] option:selected").val();
						var url = "${contextPath}/manage/seachOutStock?currentPage="
								+ currentPage
								+ "&pageSize="
								+ pageSize
								+ "&billCode=" + billCode;
