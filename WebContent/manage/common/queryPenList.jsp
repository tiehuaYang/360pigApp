                                            ...
                                            <div class="portlet box">
                                            	...
                                            	<div class="portlet-body">
		                                            <table class="table table-hover table-striped table-bordered table-advanced tablesorter tb-sticky-header">
		                                                ...
		                                                <tbody>
		                                                <c:forEach var="penVO" items="${obj.penList}" varStatus="status">
		                                                	<tr>
		                                                        <td>
		                                                            <input type="radio" name="penRadio" penId="${penVO.penId}"  penName="${penVO.penName}"/>
		                                                        </td>
		                                                        <td>${penVO.penName}</td>
		                                                        <td>${penVO.penCode}</td>
		                                                        <td>${penVO.farmPenTypeVO.penTypeName}</td>
		                                                    </tr>
		                                                </c:forEach>
		                                                </tbody>
		                                            </table>
...
<!--LOADING SCRIPTS FOR PAGE-->
    ...
    $(function()
    	...
    	function ev_return(){
    		$('input[name="penRadio"]').each(function(){
    			if( $(this).is(":checked")){
    				if(navigator.userAgent.indexOf("Chrome") >0 ){
    					//打开普通窗口的方式，通过调用父页面定义的js方法将值作为参数传递
    	    			window.opener.document.getElementById("penId").value=$(this).attr("penId");
    	    			window.opener.document.getElementById("penName").value=$(this).attr("penName");
    	    			window.close();
    	    		}
    	    		else{
    	    			var pen = new Object();
    	    			pen.penId = $(this).attr("penId");
    	    			pen.penName = $(this).attr("penName");
    	...
    	function jumpPage(currentPage){
    		...
    		var url = "${contextPath}/manage/queryPenList?currentPage="+currentPage +"&pageSize=" + pageSize;
