...
<%-- 					<jsp:include page="../common/searchBar.jsp" flush="true">
							...
							<form id="exportedForm" class="form-horizontal" method="POST">
								<div class="modal-body">
									<div class="form">
										...
										<div class="form-group">
											<label for="exportKeyword1" class="control-label col-md-3">出库单号</label>
											<div class="col-md-7">
											...
											<div class="portlet box">
													...
													<div class="row">
														<div class="col-lg-8">
															<div class="pagination-panel">
																...
																<input id="billCode" type="text" name="billCode" placeholder="出库单号" class="form-control input-sm input-inline" style="width: 100px;" value="${obj.billCode}"/> 
														...
														<div class="actions">
															<a
															href="${contextPath}/manage/newOutStorage"
												...
												<div class="portlet-body">
													<table
														...
														<thead class="success">
															<tr>
																<th class="tc" width="15%">单号</th>
																<th class="tc" width="15%">日期</th>
																<th class="tc" width="10%">类型</th>
																<th class="tc" width="20%">物料名</th>
																<th class="tc" width="10%">批次号</th>
																<th class="tc" width="10%">出库数量</th>
																<th class="tc" width="10%">制单人</th>
																<th class="tc" width="10%">操作</th>
															</tr>
														</thead>
														<%-- <tbody>
															<c:forEach var="outStorageVO"
																items="${obj.outStorageList}" varStatus="status">
																<form id="${inventoryVO.goodId.replace('-','')}">
																<tr>
																	<td>${outStorageVO.billCode}</td>
																	<td><fmt:formatDate
																			value="${outStorageVO.createTime}" type="both" /></td>
																	<td>${outStorageVO.storageType}</td>
																	<td>${outStorageVO.billMaker}</td>
																	<td>
																		<button type="button" class="btn btn-default btn-xs"
																			onclick="showBill('${outStorageVO.billId}')">
														...
														<tbody>
															<c:forEach var="outStorageVO" items="${obj.outStorageList}" varStatus="status">
																<c:if test="${outStorageVO.storageList.size() > 0}">
																	<tr>
																		<td rowspan="${outStorageVO.storageList.size()}"  style="text-align:center;vertical-align:middle">${outStorageVO.billCode}</td>
																		<td rowspan="${outStorageVO.storageList.size()}"  style="text-align:center;vertical-align:middle">
																			<fmt:formatDate	value="${outStorageVO.createTime}" type="both" />
																		</td>
																		<td rowspan="${outStorageVO.storageList.size()}"  style="text-align:center;vertical-align:middle">${outStorageVO.storageType}</td>
																		
																		<c:forEach var="storageList" items="${outStorageVO.storageList}" varStatus="status1">
					                                                   		<c:choose>
					                                                   			<c:when test="${status1.first}">
					                                                   				...
					                                                   				<td rowspan="${outStorageVO.storageList.size()}"  style="text-align:center;vertical-align:middle">${outStorageVO.billMaker}</td>
									                                                  <td rowspan="${outStorageVO.storageList.size()}"  style="text-align:center;vertical-align:middle">
																							<button type="button" class="btn btn-default btn-xs" onclick="showBill('${outStorageVO.billId}')">
																								<i class="fa fa-edit"></i>&nbsp; 查看明细
																							</button>
																						</td>
					                                                   			</c:when>
					                                                   			<c:otherwise>
					                                                   				<tr>
						                                                   				...
						                                                   				<td style="text-align:center;vertical-align:middle">${storageList.batchNumber}</td>
							                                                   		</tr>
					                                                   			</c:otherwise>
					                                                   		</c:choose>
				                                                   		</c:forEach>
    ...
    function edit(tdId,val1,val2){
    	...
    	function jumpPage(currentPage){
    		...
    		var url = "${contextPath}/manage/outStorage?currentPage=" + currentPage + "&pageSize=" + pageSize +  "&keyword=" + keyword +"&billCode=" + billCode;
        	window.location.href = url;
    	}
    	...
    	function showSelect(currentPage) {
			...
			var url = "${contextPath}/manage/OutStorage?currentPage=" + currentPage + "&pageSize="+ pageSize + "&billCode=" + billCode;
			window.location.href = url;
		}
    	...
    	function orderDownload(){
    	    ...
    	    var url = "${contextPath}/manage/exportOutStorage?billCode=" + billCode + "&startDate=" + startDate + "&endDate=" + endDate;
