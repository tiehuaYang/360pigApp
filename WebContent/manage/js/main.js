...
//    });
    ...
    var setTheme = function (menu_style, style, header, color) {
        ...
        //$('#theme-change').attr('href', '/argCom/css/themes/'+ style + '/' + color + '.css');
    }
    
    setTheme(selfTheme,null,null,null);
    ...
    $('.chat-textarea input').on("keypress", function(e){
        ...
        var $my_avt = '${contextPath}/${sessionScope.USERVO.pictureVO.uploadUrl}';
        var $your_avt = '${contextPath}/${sessionScope.USERVO.pictureVO.uploadUrl}';
        if (e.which == 13) {
            var $content = $obj.val();
...
var common_ajax = function(url,data,callback){
	...
	ajaxSet.data = data;
	ajaxSet.type  = "POST";
	ajaxSet.success = function(data){
		if(typeof(data)=="string"&&data.indexOf("{")==0)eval("data = "+data);
		if(data){
			if(callback&&typeof(callback))callback(data);
			return false;
		}
		return false;
	};
	ajaxSet.error = function(XMLHttpRequest, textStatus, errorThrown){
...
//跳转时模拟post
var common_post = function(url,data){
var form = $("<form></form>").attr("action",url).attr("method","post").appendTo("body");
data = data.split("&");
for(var i=0, input ; input = data[i] ; i++){
	input = input.split("=");
	var rowData ={
		name : input[0],
		value : input[1]
	...
	html = common_query(html,rowData);
	$(html).appendTo(form);
}
form.submit();
}
...
//字符串模板控件
function common_query(str,obj){
str=str;
for(var key in obj){
	try{
  var regex=eval("/{"+key+"}/ig");
...
function activeMenu(firstLevel,ulLevel,secondLevel){
	if(firstLevel != null && firstLevel.length>0)
		$("#"+firstLevel).addClass('active');
	if(ulLevel != null && ulLevel.length>0)
		$("#"+ulLevel).addClass('in');
